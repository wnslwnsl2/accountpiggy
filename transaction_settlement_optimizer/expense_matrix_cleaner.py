...
class ExpenseMatrixCleaner:
    def __init__(self, mat):
        ...
        self.participants = self.__get_initial_participants(
            self.expense_matrix, self.n)
    ...
    def __get_initial_mat(self, expense_log_list, n):
        ...
        for expense_log in expense_log_list:
            ...
            dividedExpense = expense_log.cost / \
                len(expense_log.participant_user_id_list)
            for participant_user in expense_log.participant_user_id_list:
                expense_matrix[self.user_id_list.index(participant_user)][self.user_id_list.index(
                    expense_log.spend_user_id)] += dividedExpense
    ...
    def __getname_order_by_total_expense_value(self, user_id_list, expense_log_list, n):
        ...
        for expense_log in expense_log_list:
            if expense_log.spend_user_id in expense_log.participant_user_id_list:
                # 자신한테 쓴 돈 제외
                expense_list[user_id_list.index(expense_log.spend_user_id)] += expense_log.cost / len(
                    expense_log.participant_user_id_list) * (len(expense_log.participant_user_id_list)-1)
            else:
                expense_list[user_id_list.index(
                    expense_log.spend_user_id)] += expense_log.cost

        # 지출 비용을 sort할 tuple
        idx_expense_value_tuple_list = [
            (idx, expense_list[idx]) for idx in range(n)]
    ...
    def __get_initial_participants(self, expense_matrix, n):
        ...
        for row in range(n):
            participants.append(Participant(
                row, expense_matrix[row], expense_matrix[:, row]))
        return participants
            ...
            for from_cand in [col for col in range(self.n) if col != sender.idx and sender.getSend(col) != 0]:
                ...
                for to_cand in [col for col in range(self.n) if col != from_cand and not (col != sender.idx and sender.getSend(col) == 0)]:
                    ...
                    while(True):
                        ...
                        canceler_idx = self.__get_canceler_idx(
                            sender, from_cand, to_cand)
                        ...
                        # 더 작은 값으로 처리한다.
                        value = min(sender.getSend(from_cand),
                                    canceler.getSend(to_cand))
